...
class Userr(AbstractUser):
    ...
    def clean(self):
        ...
        if self.leadership_role:
            ...
            existing_user_with_role = Userr.objects.filter(
                leadership_role=self.leadership_role).exclude(
                pk=self.pk).first()
    ...
    groups = models.ManyToManyField(
        'auth.Group',
        related_name='userr_groups',  # Update the related_name
        blank=True,
    ...
    user_permissions = models.ManyToManyField(
        'auth.Permission',
        related_name='userr_user_permissions',  # Update the related_name
        blank=True,
